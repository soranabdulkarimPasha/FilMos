...
MODEL_ORDER = ("FiLMoS-Net", "Hybrid CNN-LSTM", "ViT Ensemble", "DSDNet", "CTMF-Net", "SAE-Net")


def validate(frame: pd.DataFrame) -> None:
    required = {"run", "seed", "model", "accuracy", "macro_f1"}
    if set(frame.columns) != required:
        raise ValueError(f"Expected columns {sorted(required)}, found {list(frame.columns)}")
    if frame.empty:
        raise ValueError(
            "data/repeated_run_scores.csv contains only its header. Insert the 30 actual experimental rows "
            "(six models x five paired runs); summary statistics cannot reconstruct raw observations."
        )
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "results" / "paired_run_statistics.json")
